...
@Component({
  tag: 'bp-message-broker',
  shadow: false
})
export class BpMessageBroker {
   ...
   */
  @Method()
  async getLastQueued(queueName: string): Promise<object> {
    return new Promise((resolve) => {
      const queuePosition = this.__queue[queueName] ? this.__queue[queueName].msgs.length - 1 : 0
      resolve (
        !this.__queue[queueName] || !this.__queue[queueName].msgs[queuePosition]
   ...
   */
  @Method()
  async unsubscribeToQueue(queueName: string, callBackFunction: (queueValue: object) => void): Promise<boolean> {
   ...
   */
  @Method()
  async subscribeToQueue(queueName: string, callBackFunction: (queueValue: object) => void, queueType = QUEUE_TYPE_OBSERVABLE): Promise<object> {
    return new Promise((resolve, reject) => {
   ...
   */
  @Method()
  async publishToQueue(queueName: string, content: any): Promise<Array<object>> {
    return new Promise((resolve) => {
      // if there's no queue with this name create it
      if (!this.__queue[queueName]) { this._createQueue(queueName) }
  ...
  _createQueue(queueName) {
    this.__queue = {
      ...this.__queue,
      [queueName]: {
        msgs: [null], // first element needs to be null
        subscribers: [],
        broadcasting: false,
        type: undefined
      }
  ...
  _broadcastInQueue(queueName) : void {
